...
namespace RealEstate_Api.Repositories.StatisticsRepository.Concrete
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        {
            string query = "Select Name,Count(*) 'product_count' From Product Inner Join Employee On" +
                           " Product.EmployeeID=Employee.EmployeeID Group By Name Order By product_count Desc";
            using (var connection = _context.CreateConnection())
            {
        ...
        public string CategoryNameByMaxProductCount()
        {
            string query = "Select top(1) CategoryName,Count(*) From Product" +
                           " inner join Category On Product.ProductCategory=Category.CategoryID Group " +
                           "By CategoryName order by Count(*) Desc";
            using (var connection = _context.CreateConnection())
            {
                var values = connection.QueryFirstOrDefault<string>(query);
                return values;
        ...
        {
            string query = "Select Top(1) City,Count(*) as 'product_count' From Product Group By " +
                           "City order by product_count Desc";
            using (var connection = _context.CreateConnection())
            {
                var values = connection.QueryFirstOrDefault<string>(query);
                return values;
